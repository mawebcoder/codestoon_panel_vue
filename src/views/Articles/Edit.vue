  ...
  <div>
    ...
    <div class="row">
      <div class="col-6 col-768-12">
        <div
          style="text-align: center;font-weight: bold;font-size: 1.5em;margin: 20px 0"
        >
          عکس کارت
        </div>
      ...
      <div class="col-6 col-768-12">
        ...
        <DropZone
          image-name="cover"
          driver="article_category_cover_image"
          imageType="cover"
        />
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      title: "",
      category: null,
      categoryArray: [{ name: "فاقد دسته ", value: 0 }],
      tags: [],
      tagsArray: [],
    };
  },
  methods: {
    ...
    setValuesFromServer(result) {
      ...
      this.show_in_home_page = article.show_in_home_page ? true : false;
      ...
      this.status = article.status ? true : false;
    ...
    getData() {
      ...
      this.writer.trim().length ? data.append("writer", this.writer) : "";

      this.slug.trim().length ? data.append("slug", this.slug) : "";

      this.category ? data.append("article_category", this.category.value) : "";
