<template>
  <div>
    ...
    <label>
      دسته والد مورد نظر را در صورت وجود انتخاب کنید:
    </label>
    <multiselect
      selectedLabel=" "
      selectLabel="انتخاب "
  ...
  methods: {
    getInfo() {
      HttpVerbs.getRequest(`articles/categories/edit/${this.$route.params.id}`)
        .then((res) => {
          ...
          this.parentObject = result.parent
            ? (this.parentObject = {
          ...
          this.image = result.image
            ...
            : null;
          this.imageId = result.image ? (this.imageId = result.image.id) : null;
        })
        .catch((error) => {
    ...
    getSelectBox() {
      HttpVerbs.getRequest(
        `articles/categories/select-box/${this.$route.params.id}`
      )
        .then((res) => {
          if (res.status === 204) {
            return;
