  ...
  <div>
    ...
    <md-card style = "background: rgba(197,197,197,0.41)">
      <md-card-header>
        ...
        <div class = "row" style = "margin:20px 0">
          <div class = "col-6 row ">
            <template v-if = "type==='course'">عنوان دوره : {{ parent.courseTitle }}</template>
            <template v-else-if = "type==='article'">عنوان مقاله : {{ parent.articleTitle }}</template>
            <template v-else>عنوان ویدیو : {{ parent.videoTitle }}</template>
    ...
    <md-card md-with-hover v-for = "(value,index) in children" :key = "index" style = "margin: 10px 0">
      <md-card-header>
        ...
        <div class = "row" style = "margin:20px 0">
          <div class = "col-6">
            <template v-if = "type==='course'">عنوان دوره : {{ value.courseTitle }}</template>
            <template v-else-if = "type==='article'">عنوان مقاله : {{ value.articleTitle }}</template>
            <template v-else>عنوان ویدیو : {{ value.videoTitle }}</template>
          </div>
        </div>
      </md-card-header>
...
export default {
  ...
  methods: {
    getInfo()
    {
      HttpVerbs.getRequest(`comments/${this.$route.params.id}/edit`)
               .then(res =>
                     {
                       let result = res.data;
    ...
    submit()
    {
      ...
      let ApiRoutes = this.getApiRoute();
    ...
    getApiRoute()
    {
      ...
      {
        case 'course':
          return `comments/courses/${this.$route.params.id}/${this.parent.commentableId}`
        case 'article':
          return `comments/articles/${this.$route.params.id}/${this.parent.commentableId}`
        case 'video':
          return `comments/videos/${this.$route.params.id}/${this.parent.commentableId}`
      }
    },
    approve(commentId)
    {
      ...
      HttpVerbs.putRequest(`comments/switch/status`, data)
               .then(() =>
                     {
    ...
    switchChecked(commentId)
    {
      ...
      HttpVerbs.putRequest(`comments/switch/check`, data)
               .then(() =>
                     {
                       HelperClass.showSuccess(this.$noty, false)
                       this.getInfo()
